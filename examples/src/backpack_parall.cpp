...
int greedyKnapsackParallel(const std::vector<int>& weights, const std::vector<int>& values, int capacity) {
    // Разделение векторов на равные части
    int numThreads = std::thread::hardware_concurrency(); // Количество доступных ядер процессора
    int itemsPerThread = weights.size() / numThreads;

    std::vector<std::future<int>> results;
    for (int i = 0; i < numThreads; i++) {
        int start = i * itemsPerThread;
        int end = (i == numThreads - 1) ? weights.size() : start + itemsPerThread;

        // Запуск задачи в отдельном потоке
        results.push_back(std::async(std::launch::async, greedyKnapsack, 
                                     std::vector<int>(weights.begin() + start, weights.begin() + end),
                                     std::vector<int>(values.begin() + start, values.begin() + end),
                                     capacity));
    }
...
int main() {
    ...
    for (int i = 0; i < 18; i++) {  
        ...
        int maxValuePacked = greedyKnapsackParallel(weights, values, capacity);
